...
let tid_tilbage = 0
let doed = false
let bool_om_der_er_givet_point = false;
...
function draw() {
	...
	bool_om_der_er_givet_point = pointTjek(rigtige_taster)
	if (bool_om_der_er_givet_point === true || doed === true) {
		tid = millis()
	...
	drawPoints(score);
	tid_tilbage = Math.round((tid - millis()) / 1000);
...
function drawLiv(liv) {
	fill(255, 255, 255);
	rect(347, 60, 156, 50)
	if (liv >= 3) {
		image(hjerte, 347, 60, 50, 50)
	}
	if (liv >= 2) {
		image(hjerte, 400, 60, 50, 50)
	}
	if (liv >= 1) {
		image(hjerte, 453, 60, 50, 50)
	}
	if (liv === 0) {
		liv = 3 + 1;
		score = 0
